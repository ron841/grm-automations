# GRM Master Content and Sales Calendar
# Get Rooted Media LLC — Single source of truth for all content, launches, and sales milestones.
# Update this file at the end of every build session.

---

## SECTION 1: REEL LAUNCH SERIES (Ron's Personal Brand)

| Reel | Title | Status | Film Date | Post Date | Post Time | Caption Source | Hashtags Source | ManyChat Keyword |
|------|-------|--------|-----------|-----------|-----------|---------------|-----------------|------------------|
| 1 | "I Walked Away" | POSTED | March 21 | March 21 | 7:00 AM ET | content-plans/ai-as-co-founder-content-plan.md Section 3 | content-plans/ai-as-co-founder-content-plan.md Section 9 | FOUNDER |
| 2 | "What We Built" | FILMING TODAY | March 24 | March 26 | 7:00 AM ET | TBD — script to be written | TBD | None |
| 3 | "Our Third Co-Founder" | FILMING TODAY | March 24 | March 28 | 7:00 AM ET | TBD — script to be written | TBD | PLAYBOOK |
| 4 | "Why It Actually Matters" | SCHEDULED | March 27 | March 30 | 7:00 AM ET | TBD — script to be written | TBD | None |

---

## SECTION 2: AUTOMATED INSTAGRAM POSTS (Monday/Wednesday/Friday Afternoons)

**Schedule starts: Monday March 31, 2026 — AFTERNOON only**
**Note: March 30 morning belongs to Reel 4 at 7 AM. March 31 morning is clear. Automated posts go live after noon on March 31.**

Weekly pillar rotation (all posts afternoon, after 12 PM ET):
- Monday afternoon: FOUNDER
- Wednesday afternoon: LISTINGS
- Friday afternoon: NEWOWNER

Content source: output/content_calendar/ in grm-automations repo (generated Sunday 9pm by GitHub Actions).
Graphics: Built in Manus/Canva from weekly calendar content.
Posting: Manus pulls from GitHub Monday 9am, creates Canva graphics, previews to Ron, publishes on approval.

| Week | Mon PM (FOUNDER) | Wed PM (LISTINGS) | Fri PM (NEWOWNER) |
|------|-----------------|-------------------|-------------------|
| March 31 - April 4 | First automated post (March 31 PM) | April 2 PM | April 4 PM |
| April 7-11 | April 7 PM | April 9 PM | April 11 PM |
| April 14-18 | April 14 PM | April 16 PM | April 18 PM |

---

## SECTION 3: SCRIPTED REELS (Tuesday/Thursday — Ron + Cameron)

**Starts: Week of March 31**

| Date | Day | Pillar | Topic | Script Status | Source |
|------|-----|--------|-------|---------------|--------|
| March 31 | Tuesday | PLAYBOOK | Reaction to Florida AI home sale story | TO WRITE | Manus intel report March 22 |
| April 2 | Thursday | TBD | TBD based on next Manus intel report | WAITING | Manus Sunday intel March 29 |

Ongoing cadence: Two scripted reels per week (Tue/Thu), topics sourced from Manus Sunday 9:30pm intel report. Ron or Cameron films. CapCut edit with auto-captions.

---

## SECTION 4: LEAD MAGNET DROPS

| Lead Magnet | ManyChat Keyword | PDF Status | ManyChat Flow Status | Drop Date | Tied To |
|-------------|-----------------|------------|---------------------|-----------|---------|
| FOUNDER Lead Magnet v14 ("The AI Co-Founder Playbook") | FOUNDER | APPROVED | TO BUILD | Active with Reel 1 | Reel 1 CTA |
| AI Partner Playbook 1 | PLAYBOOK | TO BUILD | TO BUILD | March 28 (with Reel 3) | Reel 3 CTA |
| AI Partner Playbook 2 ("Local Business Advantage") | TBD | TO BUILD | TO BUILD | TBD | TBD |
| AI Partner Playbook 3 ("Personal Wealth and Career Toolkit") | TBD | TO BUILD | TO BUILD | TBD | TBD |
| LISTINGS Lead Magnet v7 | LISTINGS | APPROVED | TO BUILD | TBD | Automated posts |
| FOLLOWUP Lead Magnet | FOLLOWUP | TO BUILD | TO BUILD | TBD | TBD |
| NEWOWNER Lead Magnet | NEWOWNER | TO BUILD | TO BUILD | TBD | TBD |

---

## SECTION 5: SALES LAUNCH MILESTONES

| Date | Milestone | Status |
|------|-----------|--------|
| March 30 | Reel 4 posts at 7 AM. First outbound calls begin same morning. Ron and Cameron dialing. | UPCOMING |
| March 31 | Automated content posts begin (afternoon). Supporting outbound from day one. | UPCOMING |
| April 30 | Target: 20 advertisers secured across CT and FP before first print run. | TARGET |

---

## SECTION 6: SESSION LOG

Update this section at the end of every build session. One line per session.

[March 19] — GitHub repo created, content_calendar.py built, first GitHub Actions workflow live
[March 20] — Manus pipeline built, Instagram scheduler connected, editor skill automated, viral reel analysis running
[March 21] — Brand system built, FOUNDER v14 approved, Instagram graphics scheduled
[March 22] — Voice skill created, media kit editorial work, brand system fully documented
[March 24] — Workflow path bug fixed, brand system connected, Design Advisor CSV database built, Manus intel report unstuck, master calendar created

---

## SECTION 7: WEEKLY RHYTHM

Every week ongoing once launch is live:

| Time | Day | What Happens |
|------|-----|-------------|
| Sunday 9:00 PM | Sunday | GitHub Actions: content_calendar.py generates weekly content calendar automatically |
| Sunday 9:30 PM | Sunday | Manus: weekly intel report runs, saves to Drive, emails ron@getrootedmedia.com |
| Monday 7:00 AM | Monday | Scripted reel or special post (if scheduled) |
| Monday 8:00 AM | Monday | GitHub Actions: editor_skill.py polishes Monday content automatically |
| Monday afternoon | Monday | Automated pillar post: FOUNDER |
| Tuesday morning | Tuesday | Scripted reel with Ron or Cameron |
| Wednesday afternoon | Wednesday | Automated pillar post: LISTINGS |
| Thursday morning | Thursday | Scripted reel with Ron or Cameron |
| Friday afternoon | Friday | Automated pillar post: NEWOWNER |

Morning = reels and personal content. Afternoon = automated pillar posts. No overlap.

---

## SECTION 8: MANYCHAT FLOWS

Full copy and build specs in docs/MANYCHAT_FLOWS.md.

| Keyword / Trigger | Voice | What It Delivers | PDF Link | Status | Tag |
|-------------------|-------|-----------------|----------|--------|-----|
| FOUNDER | Voice 4 | The Local Media Playbook PDF + 24hr follow-up | [FOUNDER_PDF_LINK] | TO BUILD | FOUNDER-LEAD |
| PLAYBOOK | Voice 4 | The AI Partner Playbook PDF + 24hr follow-up | [AI_PLAYBOOK_LINK] | TO BUILD | PLAYBOOK-LEAD |
| LISTINGS | Voice 4 | Market shift context + CLOSING soft invite | N/A | TO BUILD | LISTINGS-LEAD |
| FOLLOWUP | Voice 4 | 90-day follow-up framework + CLOSING soft invite | N/A | TO BUILD | FOLLOWUP-LEAD |
| CLOSING | Voice 4 | The Closing Table details + Ron/Cameron personal follow-up | N/A | TO BUILD | CT-PROSPECT |
| New follower | Voice 5 | Welcome DM + routing (reply CLOSING or WELCOME) | N/A | TO BUILD | NEW-FOLLOWER |

Build order: FOUNDER first (active Reel 1 CTA), PLAYBOOK second (needed before Reel 3 posts March 28), CLOSING third, then LISTINGS, FOLLOWUP, Welcome DM.
