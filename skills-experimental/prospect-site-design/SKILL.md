---
name: prospect-site-design
description: "Generate a custom multi-page preview website for a local home services contractor prospect from their live URL. Experimental design-skill variant implementing Claude Design's editorial upgrade package — layered personas, editorial layouts, photo roles, scale contrast, motion restraint, and over-constraint escape hatch. Deeply crawls the prospect's real site, captures their brand data (owners, services, testimonials, credentials, colors, photos, promotions), scores their current web presence, picks a hero mode, picks a persona from the five-register trade map, and generates a site that composes every rule against Design's example-build.md reference. Runs isolated at ~/.claude/skills/prospect-site-design/ — does not touch production prospect-site. Use this skill whenever the user types /prospect-site-design followed by a business name and URL, or asks to build a prospect preview site under the Design editorial variant."
---
