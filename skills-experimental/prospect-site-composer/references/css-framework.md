...
:root {
  ...
  /* and any full-bleed Layout D overlay). Derived from --color-dark-primary. */
  --color-border-dark: rgba(255, 255, 255, 0.10);
  --color-text-on-dark: #F4F4F0;          /* body text on dark-primary background */
...
### Scale contrast: the display moment and the caption moment

Most template-feeling sites use three sizes (big, medium, small) in comfortable 1.25x or 1.333x ratios. The best-designed editorial sites use **extreme contrast**: one headline at 140px next to body at 16px, with nothing in between. The gap is the feeling.
...
**Scale contrast implementation rule for Phase 5:**

```
FOR each generated page:
  Compute the maximum font-size used on any element (ignoring icon glyphs)
  Compute the minimum font-size used on any text element
  ratio = max / min

  IF ratio < 5:
    Page has insufficient scale contrast. Flag to Phase 6 check 5.
    Typical fix: promote the lead h1 to display size OR add a mono caption.
```

The hero glass card naturally produces a ratio of 5-6x (72px headline over 12px eyebrow). Pages without a hero glass card (About, services, sub-pages) must establish their own display moment. Phase 5 checks this at render time and escalates to Phase 6 if it fails.
...
```css
:root {
  --rule-hairline: 1px solid var(--color-border);      /* 1px on standard DPR */
  --rule-display: 2px solid var(--color-dark-primary); /* 2px bold rule for emphasis */
  --column-gutter: 24px;                                /* Desktop gutter between columns */
...
**Rules vs boxes, as a design rule:**

Prefer hairline rules between items to boxed cards with borders. List items separated by `border-bottom: var(--rule-hairline)` read as editorial documentation. Items wrapped in rounded-corner cards with drop-shadows read as SaaS dashboard.

Sections where rules are the correct pattern (not cards):

- FAQ accordion rows — rule between questions, no card wrapping
- Footer link columns — rule between the column header and the first link
- Stats section cells — rule between stat values (vertical hairlines on desktop, horizontal on mobile)
- Services grid in list-layout variant — rule between services
- Testimonials in the editorial layout — rule above the reviewer name and date

Sections where cards remain correct:

- Services grid in Bento layout (featured 2x2 + standard cards)
- Before/after gallery frame
- Promotion callout block

The rule is not "no cards ever"; it is "use rules where rules read correctly, not cards by default."
...
- **Layout properties.** `width`, `height`, `top`, `left`, `right`, `bottom`, `margin`, `padding`, `font-size`, `grid-template-columns`. Transitioning layout forces the browser to re-layout on every frame, producing jank on anything except the newest hardware. Layout changes happen instantly or not at all.
- **Content position on scroll.** No fade-in-on-scroll animations for section content. On a service site this reads as "I can't see the information I'm looking for, the site is hiding it from me," and the prospect bounces. The scrollReveal primitive exists in the toolkit above but is applied sparingly: only to the hero entry and to the services grid on first visit. Every other section appears immediately visible.

### What may transition

Only four properties are allowed in CSS `transition` declarations:

- `color`
- `opacity`
- `border-color`
- `transform` (subject to the transform rules below)

Anything else does not transition. If a section pattern in `section-patterns.md` specifies a transition on a disallowed property, it is a bug in the pattern, not a license to add the transition.
...
### What the anti-motion rule does NOT forbid

- Hover states on buttons and links (subject to the property rules above)
- The before/after drag slider (user-initiated, not passive)
- The mobile nav drawer open/close (user-initiated)
- The FAQ accordion expand (user-initiated, height animated via `max-height` is the one exception to the layout-properties ban, because the accordion cannot function without it; animate `max-height` with `ease-out` 200ms)
- The mesh gradient canvas (slow, ambient, non-distracting — treated as still-life, not animation)
- The marquee trust strip (ambient, constant-speed, non-attention-grabbing)

Everything else is static until the user does something.
