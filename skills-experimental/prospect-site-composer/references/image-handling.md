...
Part of the prospect-site-composer skill. Loaded by Phase 3 at the start of photo capture and referenced again by Phase 5 when generating HTML with image references.
...
## Photo roles: the editorial hierarchy

A gallery is not art direction. Before Phase 3 scores or tags anything, every photo gets assigned one of four **roles**. A role is not a content-type tag (those come later and govern which section can use a photo). A role is a statement about the photo's job in the editorial system: how much weight it carries, how much space it gets, and how it relates to the photos around it.

The best-designed local sites treat photography the way a magazine does: one lead photograph per spread, supported by evidence, detail, and environment. The worst-designed local sites treat photography like a stock library: fifteen equally-weighted photos of equally-weighted subjects, arranged in a grid. This section exists to bias every build toward the first model.

### The four roles

**Role 1: Hero plate (1 per page maximum).**

The single "hero-weight" photograph on the page. Full-bleed or near-full-bleed. Large enough to establish the site's visual tone in the first 100ms. Usually desaturated, tinted toward the brand palette, or otherwise color-treated so the glass card copy sits cleanly on top.

- One hero plate per page. Not two, not three, not "one per section." One.
- On the homepage, the hero plate lives behind the Glassmorphism Trust Hero. On service sub-pages, it lives behind the sub-hero. On About, it lives behind the opening scene.
- A hero plate is almost always an environmental or work-in-progress shot, not a finished product photo. The subject is the contractor doing the work, or the workplace itself with evidence of work, not an empty "after" frame.
- Minimum resolution 1600w on the long edge. Minimum score 75 (stricter than the general 60+ bar). Aspect ratio 16:9 or wider preferred.
- Phase 3 tags these as `heroPlate` (exactly one per page that will render with a hero plate).

**Role 2: Detail shot (2-4 per build).**

Close crops of texture, material, or evidence. Water surface breaking. Solder on a connection. Tile grout on a finished bathroom. An inspection sticker on a panel. A wrench at rest on a truck tailgate.

- Detail shots are **section dividers and rhythm breakers**, not content in their own right. They appear as full-bleed strips between sections, as left-column crops beside a services list, or as watermark textures behind eyebrow labels.
- Never captioned like a portfolio photo. A detail shot is atmospheric; it carries mood, not information.
- Tight crops, shallow depth of field preferred. Resolution flexible because the crop is what matters, not the pixel count.
- Phase 3 tags these as `detailShot`.

**Role 3: Environmental portrait (1-3 per build).**

The owner or crew *working*. Not posed. Not in a matching-polo-shirt lineup. A real photograph of a real person doing their real job: at a panel, on a ladder, reading a gauge, talking to a customer on a porch.

- The subject is identifiable and specific. An environmental portrait of "Ben Tucci checking a panel in Dunnellon" is worth ten stock photos of generic electricians.
- Captioned with name + location + what they are doing. The caption is what makes it an environmental portrait rather than a generic work shot.
- Lives on the About page (primarily), the footer brand block (occasionally), and service sub-page lead sections (when the service has a specific owner-story angle).
- **Never substitutable with stock.** This is a Tier A role per the stock rules below. If Phase 3 captures zero environmental portraits, the About page uses a shop exterior or detail shot as the lead photo instead. Never a stock human.
- Phase 3 tags these as `environmentalPortrait`.

**Role 4: Evidence shot (as many as Phase 2 captures).**

Documentation photographs that prove specific claims. Before/after pairs. Certification plaques. License cards. Inspection stamps. Completed jobs with location context. Truck decals with license numbers visible.

- Evidence shots are the workhorses of a trust-building contractor site. They are not atmospheric (that is detail's job) and not lead imagery (that is hero's job). They are *receipts*.
- Always captioned with the specific fact they prove: location, date if captured, project type, license number, certification issuer.
- Populate before/after galleries, services grid cards, testimonials headshots of the reviewer's house, and the trust marquee when a credential photo exists.
- Phase 3 tags these as `evidenceShot`.

### The one-hero-weight rule

**No section on any page contains more than one hero-weight photograph.** The rest of the photos in any section are evidence, environmental portraits, or detail shots. This is the single rule that separates editorial sites from template sites.

Phase 5 enforces this at render time:

- If a section specification would render two or more photos at hero-weight (full-bleed, oversized card, or dominant visual weight), Phase 5 downgrades all but one to evidence or detail weight by cropping, sizing down, or moving to a supporting grid position.
- "Hero-weight" is defined operationally: any photo that occupies more than 40% of the viewport height at desktop, or more than 60% at mobile, counts as hero-weight.

The old failure mode this fixes: services grids with four identical large photos that all compete for attention. The fix: one featured card with a hero-weight photo, and the rest as standard cards with evidence-weight photos or icon-only treatment.

### Role assignment algorithm

Phase 3 runs role assignment AFTER quality scoring and disqualifier rejection but BEFORE content-type tagging:

```
FOR each accepted photo in the library:
  Determine subject heuristic (person / work-in-progress / finished-result / material-texture / credential / exterior / other)
  Determine composition (wide landscape / tight crop / portrait / square)
  Determine resolution tier

  IF subject is person AND composition is wide landscape AND resolution >= 1600:
    Candidate for heroPlate OR environmentalPortrait
    (decided by which pages need which — hero plate takes priority if a page needs one and has no other option)

  ELIF subject is work-in-progress AND composition is wide landscape AND resolution >= 1600 AND score >= 75:
    Candidate for heroPlate

  ELIF subject is material-texture OR tight crop:
    Tag as detailShot

  ELIF subject is person AND captioned with name:
    Tag as environmentalPortrait

  ELIF subject is credential, finished-result, or before/after pair member:
    Tag as evidenceShot

  ELSE:
    Leave role unassigned (photo still eligible for general library with evidence-default weight)
```

A single photo can hold at most one role. Multi-role tagging is not allowed because role assignment is about editorial hierarchy, not content category. A photo that could fit two roles gets the higher-priority role (hero > environmental > evidence > detail) unless another photo better fills the higher role.

### Role counts per build

Typical healthy distribution for a contractor site with 20 accepted photos:

- 1 hero plate (homepage)
- 0-3 additional hero plates (one per sub-page with a lead photo)
- 2-4 detail shots (section dividers, texture backgrounds, eyebrow watermarks)
- 1-3 environmental portraits (About page, footer, occasional service sub-page)
- 10-15 evidence shots (before/after pairs, services grid, testimonials, credentials)

A build that Phase 3 produces with zero environmental portraits and zero detail shots is over-indexed on evidence and will read as a gallery, not a site. Phase 4 surfaces this at the approval gate with the warning "Photo library is evidence-heavy. Site will feel cataloged rather than edited. Consider whether a manual environmental portrait upload would improve the build."

### Role-based section assignment (summary table)

Phase 5 uses role, not just content-type tag, when assigning photos to sections:

| Section | Primary role | Allowed fallback roles |
|---|---|---|
| Hero background (photo modes) | heroPlate | none — if no heroPlate exists, fall through to mesh hero |
| Services grid featured (2x2) | heroPlate or evidenceShot | detailShot (last resort) |
| Services grid standard cards | evidenceShot | icon-only (preferred over stock) |
| Before/after gallery | evidenceShot | no fallback — omit section if no evidence pairs |
| Testimonials headshots | no photo (Google avatar only) | never substitute |
| About page lead | environmentalPortrait | heroPlate > detailShot > exterior shot |
| About page team photos | environmentalPortrait | none |
| Footer brand block | environmentalPortrait or detailShot | none |
| Section dividers (editorial rhythm) | detailShot | none |
| Eyebrow watermark backgrounds | detailShot | none |
| Trust marquee badges | evidenceShot (credential photo) | SVG icon if no photo exists |

### Why this matters (architectural rationale)

The gallery-versus-editorial failure mode is the single biggest photo-art-direction slop signal on contractor sites. A gallery site has twelve equally-weighted 4:3 photos scattered through the page with no narrative hierarchy. An editorial site has one lead image per page, supported by detail and evidence, with clear visual rhythm.

The four-role system forces Phase 5 to make editorial decisions up front rather than letting the layout default to "put a photo in every slot." It is the photo-side equivalent of the scale contrast rule in `css-framework.md`: extreme weight ratios between elements are what make a design feel intentional rather than assembled.
...
1. **Official Instagram Graph API** — requires app registration and user-delegated auth. Only usable if the prospect has a Business or Creator account and GRM has been explicitly authorized. Not viable for cold-pipeline prospects. Defer to v0.8 Scale (deferred) when this becomes a paid client enablement feature.
...
Composer ships sites with three output formats: AVIF (modern, best compression), WebP (modern, broad support), and a single JPEG at 1200w (ultimate compatibility fallback).

**Why modern HTML needs a real fallback.** The `<picture>` element's format fallback chain requires an actual `<img>` src that every browser can render. If AVIF fails and WebP fails, the browser looks at the `<img>` src as the final fallback. If that src is itself a WebP (which is what the skill initially specified), a browser that failed on WebP sources will also fail on the WebP src, leaving a broken image. The fix is to point the `<img>` src at a format every browser has supported for 25 years: JPEG.
...
The skill uses quality 80 for WebP and AVIF at every size. JPEG fallback uses quality 85 (see JPEG rationale above) because JPEG needs slightly higher quality to match the perceived output of modern formats at 80.
...
- **Do not use quality above 85 or below 75 for compression.** 80 is the right answer.
...
image-handling.md — part of prospect-site-composer. Foundation file for Phase 3 photo pipeline. Referenced by Phase 4 (hero mode decision), Phase 5 (HTML generation), and Phase 6 (verification gates).
