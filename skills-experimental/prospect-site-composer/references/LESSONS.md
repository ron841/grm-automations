...
### 1. `--scope ron-7323` fails with "You cannot set your Personal Account as the scope"

Phase 0 (SKILL.md) still instructs the operator to treat `ron-7323` as the correct team scope for the Vercel CLI. As of April 2026 the CLI treats `ron-7323` as the personal account, not the team, and the actual deployable scope is `ron-7323s-projects` (team id `team_KCAi5VnxkXE0c5ERJWOb7jXE`). That is the same team id SKILL.md Phase 0 told the operator to AVOID as the "legacy team." The legacy / not-legacy distinction has flipped.

**Recovery that worked on this build:** `vercel link --yes --project [name] --scope ron-7323s-projects` before the deploy, then `vercel --prod --yes` (no scope flag, it picks up from `.vercel/project.json`).

**Action for v0.8:** rewrite the Phase 0 Vercel auth check against the real current team (`ron-7323s-projects`). The old `ron-7323` / `team_KCAi5VnxkXE0c5ERJWOb7jXE` taxonomy in SKILL.md is inverted.

### 2. New Vercel projects ship with SSO protection enabled by default

The initial deploy returned HTTP 401 with a `_vercel_sso_nonce` cookie on the raw deployment URL. SKILL.md Phase 7 assumes public deployment URLs. They are no longer public by default on team accounts.

**Recovery that worked:** `curl -X PATCH https://api.vercel.com/v9/projects/{id}?teamId={team} -H "Authorization: Bearer $TOKEN" -d '{"ssoProtection": null}'`, using the token from `~/Library/Application Support/com.vercel.cli/auth.json`. After the PATCH the URL returned HTTP 200 immediately. `vercel alias set` to the `[slug]-grm.vercel.app` canonical domain worked as spec'd.

**Action for v0.8:** add Phase 7 step 4.5: after deploy, automatically PATCH `ssoProtection: null` on the project, then alias. Or set `ssoProtection: null` at project-create time. Without this, every prospect we text the URL to sees a Vercel login screen and we look broken.

### 3. AI-slop stock imagery on the prospect site was rejected correctly

The two "owned" jpegs on the prospect site (`3a0cf01b-…jpeg`, `f089d93b-…jpeg`) were both AI-generated stock art (a luxury living room and a clock+calendar illustration), not Luna's real paint work. Phase 3's visual inspection caught this. Google Places provided 10 real owned photos that replaced them fully. The two site jpegs were logged as rejected with reason in `profile.json.photos.site.rejected = 2`.

**Action for v0.8:** none required. This is the system working. The Phase 3 rule "Prospect's own website photos must be watermark-free, logo-free, high-resolution" already catches this class implicitly, and the current Phase 3 tagging (`content: 'stock'` or `'unknown'` disqualifier) handles it correctly. Worth noting because it is a pattern that will repeat on GoDaddy-hosted sites where owners have clearly asked ChatGPT for "living room photos" rather than uploading their own work.

### 4. Brand-token override: auto-derived dark-primary was wrong for the logo

Auto-derivation produced `#742a14` (dark burnt orange) as `--color-dark-primary`. The actual logo contains navy `#1c2667` (DarkVibrant swatch, pop=7) as a co-primary color. The navy is visibly used in "MASTER PAINTING" text, house outlines, and the crescent moon. Burnt-orange headings would be off-brand.

**Decision:** override `--color-dark-primary` to `#1c2667` manually, matching the node-vibrant DarkVibrant swatch per SKILL.md Phase 1 step 5 semantic mapping, instead of using the auto-derive function from `css-framework.md`.

**Action for v0.8:** the `deriveDarkPrimary()` function is wrong when DarkVibrant has nonzero population AND population >= 5. Consider: when `DarkVibrant.population >= 5`, use `DarkVibrant.hex` as `--color-dark-primary` directly; fall back to `deriveDarkPrimary(primary)` only when DarkVibrant is absent or has population 0-4. This would handle brands with deliberate two-color logos (Luna, Tucci, T&F) correctly out of the box.
