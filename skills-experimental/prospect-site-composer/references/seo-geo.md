...
Part of the prospect-site-composer skill. Loaded by Phase 5 during HTML generation and Phase 5.5 during structured data injection. Referenced by Phase 6 for verification.
...
- **Keyword research and content marketing strategy.** GRM may offer this as a paid add-on, but the skill focuses on the markup and structural baseline, not on content strategy.
...
The skill's job is to produce sites that start from a structurally strong AI-citation position on the day they ship. Ongoing optimization is a client relationship, not a generator task.
...
**For Composer builds,** Phase 5 writes the build date as lastmod on every page. This is accurate on the day the site ships.
...
The spec also defines `llms-full.txt`, a comprehensive single-document version of the entire site content. The skill does NOT generate llms-full.txt because (a) AI systems rarely fetch it, (b) it duplicates content that already exists on the regular HTML pages, and (c) the marginal impact is not worth the build complexity. If adoption data shifts in a later release, the decision can be revisited.
...
**What GRM controls:**
- Allow all three crawler identities in robots.txt (default)
...
**What GRM controls:**
- Allow all four crawler identities in robots.txt (default)
...
**What GRM controls:**
- Allow PerplexityBot in robots.txt (default)
...
**What GRM controls:**
- Bingbot access in robots.txt (default)
...
**What GRM controls:**
- Google-Extended access in robots.txt (default)
...
The AI citation research surfaced several high-value tactics that Composer does not implement. This section names them explicitly so nothing gets silently dropped. Each one is either a v0.8 Scale (deferred) or later roadmap item, or a deliberate scope boundary. When these tactics appear in client conversations or sales pitches, GRM should be able to speak to them with confidence and explain the reasoning.
...
**Why the skill does not do this.** Backlink work is an ongoing operational activity, not a skill output. It is also fundamentally contrary to the strategic thesis of this file: the 17 percent overlap finding means AI citations do not care about backlinks. GRM is building for the new game, not the old one.
...
**Why the skill does not do this.** Content marketing is a separate recurring engagement, not a one-time site build. GRM may offer it as a paid add-on in the future, but it is not part of the Composer skill.

**Roadmap.** v0.8 Scale (deferred) may introduce a quarterly content refresh workflow that ties into the 13-week freshness cycle (documented in `deployment.md`). That workflow would generate a small number of fresh pages per quarter per client based on seasonal topics and captured site data. Not a full content marketing service, but enough to maintain the freshness signal that AI crawlers reward.
...
**Why the skill does not do this.** Monitoring is an ongoing client services activity, not a generator task. Full implementation is documented in `deployment.md` under the monitoring stack. The GEO tool landscape in this file gives GRM a starting point for tool selection.
...
**Why the skill does not do this.** Marion County contractors GRM is targeting are overwhelmingly single-location operators. Adding multi-location support would complicate the skill without matching the market reality.

**Roadmap.** If GRM wins a multi-location client (e.g., an HVAC chain with locations in Ocala, Gainesville, and The Villages), a v0.8 Scale (deferred) extension can add per-location page generation. Not a Composer target.
