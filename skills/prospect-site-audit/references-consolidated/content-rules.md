...
2. **Documents the universal GRM rules and banned words list** as a synced excerpt so Phase 5 can enforce them without network access. This excerpt must stay in sync with the voice skill manually. When the voice skill updates, update this file too. **Sync direction is one-way: GRM_VOICE_SKILL.md → this file (and the parallel copy at `~/grm-design-handoffs/voice/`). Never edit the banned-words/voice-rule excerpt below to override the upstream; if a rule is wrong, fix it in GRM_VOICE_SKILL.md and re-sync the excerpt here.**
