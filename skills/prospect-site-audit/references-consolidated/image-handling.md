...
### Per-photo metadata (`photoLibrary.photos[<rel_path>]`)

Per-photo metadata is keyed by the photo's relative path under `photos_root` and lives at `profile.photoLibrary.photos`. The map is keyed by the same path string the photo-reference slots (`heroCandidatesTop5[]`, `ownerPortrait`, `teamIdentity`, `services.list[].featuredImage`, etc.) use, so cross-references match without normalization. Existing slot shapes stay unchanged — slots remain string paths; this map is parallel.
...
"photoLibrary": {
  "heroCandidatesTop5": [...],
  "ownerPortrait": "assets/photos/site/portrait.jpg",
  "teamIdentity": "assets/photos/site/family.jpg",
  "photos": {
    "assets/photos/site/family.jpg": {
      "category": "team",
      "luminance": 0.51,
      "disqualifyFromHero": false,
      "disqualifyReason": ""
    },
    "assets/photos/google/places_00.jpg": {
      "category": "award",
      "disqualifyFromHero": true,
      "disqualifyReason": "Best of 2024 plaque, not a finished-work photo"
    }
  }
}
```

All four fields are independently optional; absent fields fall through to legacy / default behavior.

**`category` — locked vocabulary, caption-photo integrity gate (v1.1 Session 2 Patch 3, 2026-04-30).** Authors tag at Phase 4 photo curation. The engine consults the tag at band emit. Allowed values:

- `team` — multi-person crew shot, family-business group photo, branded-apparel team photo
- `owner-portrait` — single owner / founder portrait, identity confirmed
- `finished-work` — completed project shot (the "after" shot, the showcase shot)
- `exterior` — building / shop exterior, signage, storefront
- `interior` — building / shop interior, workshop, behind-the-counter
- `equipment` — trucks, tools, vehicles with branding
- `award` — plaque, certificate, magazine cover, recognition shot
- `before-after` — pair-member or composite showing before-and-after state
- `other` — unclassified but accepted (rarely used)

**Gate semantics — missing = legacy, wrong = drop.** A teamIdentity slot pointing at a photo with no `category` field emits (engine trusts author placement). A teamIdentity slot pointing at a photo tagged anything other than `team` drops the band cleanly — never substitute caption-on-mismatched-photo (the Goney's pre-patch defect: "On The Trucks / Fifty-three years" caption shipped against a greenhouse-poinsettias single-worker photo because the caption template ran without a category check). Category mismatch at any tier in a fall-through chain (e.g., ownerPortrait → teamIdentity → site-04) drops the entire band rather than fall to the next tier — caption was authored for the asserted tier's context.

Bands gated today: story-grid portrait band (about page) — ownerPortrait tier requires `owner-portrait`, teamIdentity tier requires `team`. Future bands extend the gate as they ship.

**`luminance` — mean photo brightness 0–1 (v1.1 Session 2 Patch 1, 2026-04-30).** Drives the adaptive overlay opacity in Patch 2. Engine lazy-computes via Pillow when absent and writes back to the in-memory profile dict (NOT to disk). Authors don't normally write this manually; it's engine-derived metadata that appears in the dict after first render.

**`disqualifyFromHero` (boolean, default false) + `disqualifyReason` (optional string, documentation only) — hero curation respect (v1.1 Session 2 Patch 4, 2026-04-30).** Authors flip `disqualifyFromHero: true` at Phase 4 photo curation when a photo is in `heroCandidatesTop5` but shouldn't actually fire as a hero rotation slide — typical cases: marketing collages with watermarks, award plaques, before-after composites, anything with self-published branding. The hero rotation resolver filters these out before crossfade emit. `disqualifyReason` is optional and documentation-only — useful for future audit traces ("why did this photo get disqualified?") but the engine doesn't read it. If filtering drops the candidate count below the crossfade threshold (≥3), the engine falls through to single-photo or text-only hero per existing logic.
